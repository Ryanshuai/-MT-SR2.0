...
parser.add_argument('--crop_size', default=768, type=int, help='training images crop size')
...
for epoch in range(1, NUM_EPOCHS + 1):
    ...
    netG.train()
    netD.train()
    for x, target, mask in train_bar:
        mask = Variable(mask)
        if torch.cuda.is_available():
            mask = mask.cuda()
        ...
        ###########################
        netG.zero_grad()
        g_loss = generator_criterion(fake_out, fake_img, real_img, mask)
        g_loss.backward()
        optimizerG.step()
        fake_img = netG(z)
        fake_out = netD(fake_img).mean()
    ...
    val_images = torch.chunk(val_images, val_images.size(0) // 14)  #contral im munbers in one grid
