...
im = im.convert('RGBA')
im_blender = Image.new('RGBA', im.size, (0,0,0,0))
blender_draw = ImageDraw.Draw(im_blender)
...
for i in range(6):
    ...
    blender_draw.polygon([x0, y0, x1, y1, x1, 0, x0, 0], fill="black")
for i in range(6, 11):
    x0, y0, x1, y1 = mp[i][0], mp[i][1], mp[i+1][0], mp[i+1][1]
    blender_draw.polygon([x0, y0, x1, y1, x1, im.height, x0, im.height], fill="black")
x0, y0, x1, y1 = mp[11][0], mp[11][1], mp[0][0], mp[0][1]
blender_draw.polygon([x0, y0, x1, y1, x1, im.height, x0, im.height], fill="black")

# fill with white left and right
blender_draw.rectangle([0, 0, left, im.height], fill="black")
blender_draw.rectangle([right, 0, im.width, im.height], fill="black")

im_blender.show()

masked_im = Image.composite()

masked_im.show()
masked_im.save('tttttt.png')
